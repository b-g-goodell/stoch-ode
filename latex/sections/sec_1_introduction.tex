...
The ODE is separable and thus solvable using ODE techniques (not a prerequisite for this chapter.)
...
Worse, the model requires high precision to parameterize. We say it is sensitive. For the rabbit population, we can estimate $r$ by using the two cases of rabbit breeding speed above. For the $P(2)=8$ case, $P(t) = 2 e^{rt}$ implies $P(2)=2e^{2r}$. Solving for $r$ gives $r = \frac{1}{2}\ln(4) = \frac{2\ln(2)}{2} \approx 0.69315$. On the other hand, for the $P(6)=2e^{6r}=8$ case, a similar argument suggests that $r = \frac{1}{6}\ln(4) = \frac{\ln(2)}{3} \approx 0.23105$. Both of these are estimates for a per-capita birth rate $r$,  a rabbit population will see average behavior somewhere between these two extremes. But, these are two very different values. A population with $r \approx 0.69315$ doubles about thrice as often as a population with $r \approx 0.23105$.
...
\centering
\includegraphics[width=\textwidth]{17StochasticODEs/stoch-ode/latex/figures/radioactive_decay.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{17StochasticODEs/stoch-ode/latex/figures/logistic_growth_mean_vs_ode.pdf}
...
practical starting point. If you installed \textit{Git}, the following command downloads the repository into a new directory named \texttt{stoch-ode} automatically.
\begin{verbatim}
git clone https://github.com/b-g-goodell/stoch-ode
\end{verbatim}

Or, navigate to \url{https://github.com/b-g-goodell/stoch-ode}, see the repository directory structure plainly, and use \textit{Code $\to$ Download ZIP} to obtain the entire repository without \textit{Git}.
...
\texttt{simulation\_frameworks/}& One framework per model of Section~\ref{sec:farming} \\
...
A special thanks must be extended to Diego Salazar at Cypher Stack and Power-Up Privacy for helping to make this chapter possible, as well as Joshua Babb at Cypher Stack for myriad helpful conversations.
